...
begin
 ...
 if p_reply_to_voice_id is not null then select name,'Voice message' into target_name,target_preview from public.private_voice_messages where id=p_reply_to_voice_id and room_id=p_room_id and expires_at>now(); if target_name is null then raise exception 'Invalid voice reply target'; end if; end if;
...
begin
 ...
 insert into public.private_voice_messages(room_id,user_id,name,country,subdivision,avatar_id,audio_url,storage_path,duration_ms,reply_to_voice_id,reply_to_preview,reply_to_name) values(p_room_id,auth.uid(),left(trim(p_name),32),left(trim(p_country),80),left(trim(p_subdivision),120),p_avatar_id,p_audio_url,p_storage_path,p_duration_ms,p_reply_to_voice_id,case when p_reply_to_voice_id is not null then 'Voice message' end,target_name) returning * into v; return v;
end; $$;
