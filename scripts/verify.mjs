...
ok('private clean shield/lock animation',/private-shield-animation/.test(app)&&/ShieldCheck/.test(app)&&/LockKeyhole/.test(app)&&!privateShieldVideoPattern.test(app));
...
ok('public voice manual send/cancel flow',/recordedVoiceBlob/.test(app)&&/sendRecordedVoice/.test(app)&&/cancelRecording/.test(app)&&/voice-finish/.test(app)&&!/recorder\.onstop=async/.test(app));
...
ok('no release .env',!fs.existsSync(path.join(root,'.env')));
