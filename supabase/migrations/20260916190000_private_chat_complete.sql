...
create table if not exists public.private_message_reactions (
  ...
  reaction text not null check(reaction in ('👍','❤️','😂','😮','😢','😡','🎉','🙏')), created_at timestamptz not null default now(), primary key(message_id,user_id,reaction)
);
create table if not exists public.private_voice_reactions (
  ...
  reaction text not null check(reaction in ('👍','❤️','😂','😮','😢','😡','🎉','🙏')), created_at timestamptz not null default now(), primary key(voice_id,user_id,reaction)
);
