...
import { ArrowDown, Check, ChevronDown, Gamepad2, MicOff, Reply as ReplyIcon, Send, Settings, Smile, Trash2, Volume2, Wifi, WifiOff, X } from 'lucide-react';
...
function Chat({profile,settings,onSettings,onProfile}:{profile:Profile;settings:{textSize:TextSize;sound:boolean;timeFormat?:TimeFormat};onSettings:(x:any)=>void;onProfile:(x:Profile)=>void}){
    ...
    (async()=>{
      ...
      channel
        ...
        .on('postgres_changes',{event:'INSERT',schema:'public',table:'messages'},(payload:any)=>{
          ...
          if(m.user_id!==selfId)play('receive');
        })
  ...
  useEffect(()=>{const el=listRef.current;if(el&&shouldStickToBottom())el.scrollTo({top:el.scrollHeight,behavior:'smooth'});else if(messages.length) setShowJump(true)},[messages.length]);
  const handleScroll=()=>{const el=listRef.current;if(!el)return;const near=el.scrollHeight-el.scrollTop-el.clientHeight<120;setShowJump(!near&&messages.length>0)};
  const jumpToLatest=()=>{const el=listRef.current;if(!el)return;el.scrollTo({top:el.scrollHeight,behavior:'smooth'});setShowJump(false)};
  ...
  return <div className="chat-app"><header className="topbar"><div className="top-brand"><div className="brand-mini">🌍</div><div><b>GLOBAL CHAT</b><span><i className={connected?'online-dot':'offline-dot'}></i>{online} Online</span></div></div><div className="top-actions"><button className="clear-chat-btn" onClick={()=>setClearConfirmOpen(true)}><Trash2 size={17}/> CLEAR CHAT</button><button className="settings-btn" onClick={()=>setSettingsOpen(true)}><Settings size={18}/> SETTINGS</button></div></header>
    ...
    <main className="chat-main"><div className="message-list" ref={listRef} onScroll={handleScroll}>{messages.length===0?<div className="empty"><div className="empty-earth">🌍</div><h2>Talk to the world</h2><p>Say hello. Share ideas. Keep it friendly.</p></div>:messages.map(m=><Message key={m.id} m={m} current={m.user_id===authUserId} timeFormat={settings.timeFormat || '12h'} counts={reactionCounts[m.id]||{}} mine={myReactions[m.id]||[]} onToggle={async(r)=>{const {error}=await supabase.functions.invoke('toggle-reaction',{body:{messageId:m.id,reaction:r}});if(!error)refreshReactions(messageIdsRef.current,authUserId)}} onDelete={()=>deleteLocally(m)} onReply={()=>beginReply(m)}/>)}</div>{showJump&&<button className="jump-latest" onClick={jumpToLatest}><ArrowDown size={15}/> NEW MESSAGES</button>}{typingUsers.length>0&&<div className="typing-indicator">Someone is typing<span className="typing-dots"><i></i><i></i><i></i></span></div>}
