...
import { ArrowDown, Bell, BookOpen, Check, ChevronDown, Clock3, Copy, Globe2, Image, KeyRound, LockKeyhole, Mic, MicOff, MoreVertical, Palette, Pause, Pencil, Play, Plus, Reply as ReplyIcon, Search, Send, Settings, Smile, Trash2, UserRound, Users, Volume2, Wifi, WifiOff, X } from 'lucide-react';
  ...
  return <div className="feedback-backdrop" role="dialog" aria-modal="true" aria-labelledby="feedback-title">
    <div className="feedback-panel">
      ...
      {done ? <div className="feedback-success"><div className="feedback-success-icon"><Check size={22}/></div><h3>Report submitted</h3><p>Thanks. Your report has been sent for review.</p><button className="primary-btn" onClick={onClose}>DONE</button></div> : <>
        ...
        <div className="feedback-types"><button className={type==='bug'?'active':''} onClick={()=>setType('bug')}>🐛 Bug / Error</button><button className={type==='feedback'?'active':''} onClick={()=>setType('feedback')}>💬 Feedback</button><button className={type==='not-working'?'active':''} onClick={()=>setType('not-working')}>⚠️ Something not working</button></div>
