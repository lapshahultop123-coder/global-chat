...
import { ArrowDown, Bell, BookOpen, Check, ChevronDown, Clock3, Copy, Gamepad2, Globe2, Image, KeyRound, LockKeyhole, Mic, MicOff, MoreVertical, Palette, Pause, Pencil, Play, Plus, Reply as ReplyIcon, Search, Send, Settings, ShieldCheck, Smile, Trash2, UserRound, Users, Volume2, Wifi, WifiOff, X } from 'lucide-react';
...
function Chat({profile,settings,onSettings,onProfile}:{profile:Profile;settings:{textSize:TextSize;sound:boolean;timeFormat?:TimeFormat};onSettings:(x:any)=>void;onProfile:(x:Profile)=>void}){
  ...
  const [replyTargetForPrivate,setReplyTargetForPrivate]=useState<any>(null);
  ...
  const startRecording=async()=>{if(recording||sending)return;if(!navigator.onLine){setError('You are offline. Please reconnect before recording.');return}const stream=await requestMic();if(!stream)return;try{const mime=['audio/webm;codecs=opus','audio/webm','audio/mp4'].find(x=>MediaRecorder.isTypeSupported(x))||'';const recorder=new MediaRecorder(stream,mime?{mimeType:mime,audioBitsPerSecond:24000}:undefined);mediaRecorderRef.current=recorder;mediaChunksRef.current=[];recordingStartedRef.current=Date.now();setRecordingSeconds(0);setRecordedVoiceBlob(null);setRecordedVoiceDuration(0);setError('');setRecording(true);broadcastPublicRecording(true);recorder.ondataavailable=e=>{if(e.data.size)mediaChunksRef.current.push(e.data)};recorder.onstop=()=>{stream.getTracks().forEach(t=>t.stop());const elapsed=Math.min(60000,Date.now()-recordingStartedRef.current);const blob=new Blob(mediaChunksRef.current,{type:recorder.mimeType||'audio/webm'});mediaChunksRef.current=[];if(elapsed<500){setError('Voice message is too short.');setRecordingSeconds(0);return}if(blob.size>350*1024){setError('Voice message is too large. Please record a shorter message.');setRecordingSeconds(0);return}setRecordedVoiceBlob(blob);setRecordedVoiceDuration(elapsed);setRecordingSeconds(Math.floor(elapsed/1000));};recorder.start(250);recordingTimerRef.current=window.setInterval(()=>{const elapsed=Math.floor((Date.now()-recordingStartedRef.current)/1000);if(elapsed>=60){stopRecording();return}setRecordingSeconds(elapsed)},200)}catch{stream.getTracks().forEach(t=>t.stop());setRecording(false);setError('Could not start microphone recording. Please try again.')}};
  ...
  const beginVoiceReply=(v:VoiceMessage)=>{const target:ChatMessage={id:`voice-reply-${v.id}`,user_id:v.user_id,name:v.name,country:v.country,subdivision:v.subdivision,avatar_id:v.avatar_id,body:'Voice message',created_at:v.created_at,expires_at:v.expires_at,reply_to_voice_id:v.id};setReplyTarget(target);requestAnimationFrame(()=>document.querySelector<HTMLTextAreaElement>('.composer textarea')?.focus())};
  ...
  const enterPrivateRoom=async(room:any)=>{
    ...
    const ch:any=supabase.channel('private-room-'+room.id,{config:{presence:{key:authUserId},broadcast:{self:false,ack:true}}})
      ...
      .on('postgres_changes',{event:'INSERT',schema:'public',table:'private_messages',filter:'room_id=eq.'+room.id},
        payload=>setPrivateMessages(prev=>prev.some(m=>m.id===payload.new.id)?prev:[...prev,payload.new].sort((a,b)=>a.created_at.localeCompare(b.created_at))))
          ...
          {recording||recordedVoiceBlob
            ? <div className="voice-recorder public-voice-recorder public-style-voice-recorder">
                <div className="voice-rec-left">
                  ...
                  <div className="recording-wave" aria-label="Recording waveform">{Array.from({length:28},(_,i)=><i key={i} className={recording?'active':''} style={{height:`${7+(i%7)*2}px`}}></i>)}</div>
                </div>
                ...
                {recording
                  ? <button className="voice-send voice-finish" onClick={stopRecording} aria-label="Finish recording" title="Finish recording"><Check size={18}/></button>
...
const Message=memo(function Message({m,current,counts,mine,onToggle,onDeleteForMe,onDeleteForEveryone,onReply,onCopy,onReplyJump,highlighted,copied,timeFormat}:{m:ChatMessage;current:boolean;counts:Record<string,number>;mine:string[];onToggle:(r:string)=>Promise<void>;onDeleteForMe:()=>void;onDeleteForEveryone:()=>void;onReply:()=>void;onCopy:()=>void;onReplyJump:(id:string)=>void;highlighted:boolean;copied:boolean;timeFormat:TimeFormat}){
  ...
  return <article id={`msg-${m.id}`} className={`private-message public-message ${current?'mine':''} ${highlighted?'highlighted':''}`}>
    ...
    {m.reply_to_id&&<button className="reply-preview" onClick={()=>onReplyJump(m.reply_to_id!)}><ReplyIcon size={13}/><span><b>{m.reply_to_name||'User'}</b>{m.reply_to_preview||'Reply'}</span></button>}
...
function PrivateRoomView({profile,room,messages,setMessages,text,setText,sending,onSend,onClose,onCopy,replyTarget,setReplyTarget,currentUserId,sendError,privateOnline,privateTyping,onTyping,sound,onMembers,privateRecording,onRecording}:{profile:Profile;room:any;messages:any[];setMessages:React.Dispatch<React.SetStateAction<any[]>>;text:string;setText:(v:string)=>void;sending:boolean;onSend:()=>void;onClose:()=>void;onCopy:(m:any)=>void;replyTarget:any;setReplyTarget:(v:any)=>void;currentUserId:string;sendError:string;privateOnline:number;privateTyping:boolean;onTyping:(value:string)=>void;sound:boolean;onMembers:()=>void;privateRecording:boolean;onRecording:(active:boolean)=>void}){
  ...
  const uploadPrivateVoice=async(blob:Blob,duration:number)=>{
    ...
    try{
      ...
      const {data,error}=await supabase.rpc('send_private_voice',{p_room_id:room.id,p_name:profile.name,p_country:profile.country,p_subdivision:profile.subdivision,p_avatar_id:profile.avatarId,p_audio_url:path,p_storage_path:path,p_duration_ms:duration,p_reply_to_voice_id:replyTargetForPrivateRef.current?.reply_to_voice_id||null});
      if(error){await supabase.storage.from('private-voice-messages').remove([path]);throw error;}
  ...
  const startVoice=async()=>{
    ...
    try{
      ...
      chunksRef.current=[];startedRef.current=Date.now();setRecordedVoiceBlob(null);setRecordedVoiceDuration(0);setPrivateError('');
      ...
      rec.onstop=()=>{
        ...
        if(elapsed<500){setRecordingSeconds(0);setPrivateError('Voice message is too short.');return}
        if(blob.size>320*1024){setRecordingSeconds(0);setPrivateError('Voice message is too large. Please record a shorter message.');return}
      ...
      timerRef.current=window.setInterval(()=>{const elapsed=Math.floor((Date.now()-startedRef.current)/1000);if(elapsed>=60){stopVoice();return}setRecordingSeconds(elapsed)},250);
    }catch(e:any){stream?.getTracks().forEach(t=>t.stop());setRecording(false);setRecordingSeconds(0);setPrivateError(e?.name==='NotAllowedError'?'Microphone permission was denied. Please allow microphone access.':(e?.message||'Microphone access is unavailable.'));}
  };
  const stopVoice=()=>{const rec=recorderRef.current;if(!rec||rec.state==='inactive')return;onRecording(false);rec.stop()};
  const cancelVoice=()=>{
    onRecording(false);
    ...
    setRecording(false);setRecordingSeconds(0);setRecordedVoiceBlob(null);setRecordedVoiceDuration(0);setVoiceBusy(false);setPrivateError('');
  };
  ...
  const beginPrivateReply=(item:any,isVoice=false)=>{const target=isVoice?{...item,id:`voice-reply-${item.id}`,reply_to_voice_id:item.id,body:'Voice message'}:item;setReplyTargetForPrivate(target);setText('');requestAnimationFrame(()=>document.querySelector<HTMLTextAreaElement>('.private-composer textarea')?.focus())};
  ...
  return <div className="private-room-shell">
    ...
    <div className="private-message-list">
      {all.length===0?<div className="empty private-empty-state"><div className="private-shield-animation" aria-hidden="true"><ShieldCheck className="private-shield-icon" size={104} strokeWidth={1.45}/><LockKeyhole className="private-lock-icon" size={34} strokeWidth={2.1}/></div><h2>Private chat</h2><p>Only joined members can see these messages.</p></div>:
      all.map(item=>item.kind==='text'?
        <article key={'m-'+item.data.id} id={'pm-'+item.data.id} className={`private-message ${item.data.user_id===currentUserId?"mine":""}`}>
          ...
          {(item.data.reply_to_id||item.data.reply_to_voice_id)&&<button className="reply-preview" onClick={()=>document.getElementById(item.data.reply_to_voice_id?'pv-'+item.data.reply_to_voice_id:'pm-'+item.data.reply_to_id)?.scrollIntoView({behavior:'smooth',block:'center'})}><ReplyIcon size={13}/><span><b>{item.data.reply_to_name||'User'}</b>{item.data.reply_to_preview||'Reply'}</span></button>}
          ...
          <div className="private-item-actions">
            ...
            {menuId==='m'+item.data.id&&<div className="private-item-menu"><button onClick={()=>{beginPrivateReply(item.data);setMenuId(null)}}><ReplyIcon size={14}/> Reply</button><button onClick={()=>deleteForMe(item.data.id,false)}>Delete for me</button><button className="danger" onClick={()=>void deleteForEveryone(item.data.id,false)}>Delete for everyone</button><button onClick={()=>setMenuId(null)}><X size={14}/> Cancel</button></div>}
          </div>
        </article>
      :
        <article key={'v-'+item.data.id} id={'pv-'+item.data.id} className={`private-message private-voice-message ${item.data.user_id===currentUserId?"mine":""}`}>
          <div className="message-identity"><img className="message-avatar" src={avatarSrc(Number(item.data.avatar_id))} alt=""/><div><strong>{item.data.name}</strong><span>{flag(item.data.country)} {item.data.subdivision}</span></div></div>
          {item.data.reply_to_voice_id&&<button className="reply-preview voice-reply-preview" onClick={()=>document.getElementById('pv-'+item.data.reply_to_voice_id)?.scrollIntoView({behavior:'smooth',block:'center'})}><ReplyIcon size={13}/><span><b>{item.data.reply_to_name||'User'}</b>{item.data.reply_to_preview||'Voice message'}</span></button>}
    ...
    {replyTargetForPrivate&&<div className="reply-composer private-reply"><div><b><ReplyIcon size={14}/> Replying to {replyTargetForPrivate.name}</b><span>{replyTargetForPrivate.body}</span></div><button className="private-modal-cancel" onClick={()=>setReplyTargetForPrivate(null)} aria-label="CANCEL reply"><X size={16}/></button></div>}
    ...
    {recording||recordedVoiceBlob?
      <div className="voice-recorder private-voice-recorder public-style-voice-recorder">
        ...
        {recording?
          <button className="voice-send voice-finish" onClick={stopVoice} aria-label="Finish recording" title="Finish recording"><Check size={18}/></button>
          :
          <button className="voice-send" onClick={()=>void sendRecordedPrivateVoice()} disabled={voiceBusy} aria-label="Send voice message" title="Send voice message"><Send size={18}/></button>}
      </div>
      :
      <div className="composer private-composer">
        <div className="tools private-tools"><button className="icon-btn" onClick={()=>setPrivateEmojiOpen(v=>!v)} aria-label="Emoji"><Smile size={19}/></button>{privateEmojiOpen&&<div className="private-emoji-pop">{["😀","😂","😍","🥰","😎","🤩","😱","😭","😡","👍","👏","🙏","🔥","❤️","💯","🎉"].map(e=><button key={e} onClick={()=>{setText(text+e);setPrivateEmojiOpen(false)}}>{e}</button>)}</div>}</div>
        ...
        rooms.map(room=>{
          ...
          return <div className="your-private-chat-item" key={room.id}>
            ...
            <div className="your-private-chat-actions">
              ...
              {owner&&<button className="your-private-rename-btn" onClick={()=>onRename(room)} aria-label="Rename private chat" title="Rename private chat">✎</button>}
