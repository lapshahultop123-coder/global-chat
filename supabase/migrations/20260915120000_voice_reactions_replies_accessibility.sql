...
create table if not exists public.voice_reactions (
  ...
  reaction text not null check (reaction in (U&'\+1F44D',U&'\+2764\+FE0F',U&'\+1F602',U&'\+1F62E',U&'\+1F622',U&'\+1F621',U&'\+1F389',U&'\+1F64F')),
...
begin
  ...
  if p_reaction not in (U&'\+1F44D',U&'\+2764\+FE0F',U&'\+1F602',U&'\+1F62E',U&'\+1F622',U&'\+1F621',U&'\+1F389',U&'\+1F64F') then raise exception 'invalid_reaction'; end if;
...
begin
  ...
  if p_reply_to_voice_id is not null then
    ...
    update public.messages set reply_to_voice_id=p_reply_to_voice_id,reply_to_id=null,reply_to_preview='Voice message',reply_to_name=(select name from public.voice_messages where id=p_reply_to_voice_id) where id=v.id returning * into v;
  end if;
  return v;
end;
$$;
...
begin
  ...
  values(p_user_id,trim(p_name),p_country,p_subdivision,p_avatar_id,p_audio_url,p_storage_path,p_duration_ms,p_file_size,p_reply_to_voice_id,case when p_reply_to_voice_id is not null then 'Voice message' end,case when p_reply_to_voice_id is not null then (select name from public.voice_messages where id=p_reply_to_voice_id) end)
  returning * into v;
  return v;
end;
$$;
